...
pokemonsRouter.get('/', async (req, res) => {
	...
	try {
		...
		const completePokemons = await Promise.all(
			pokemons.map(async (pokemon: CompletePokemon): Promise<Object> => {
...
pokemonsRouter.get('/bytype', async (req, res) => {
	...
	try {
		...
		const completePokemons = await Promise.all(
			pokemons.map(async (pokemon: PokemonsByType): Promise<Object> => {
...
pokemonsRouter.get('/filter', async (req, res) => {
	...
	try {
		...
		const completePokemons = await Promise.all(
			filteredPokemons.filter(Boolean).map(async (pokemon: CompletePokemon): Promise<Object> => {
				const completePokemon = await getPokemon(pokemon.url);
				return completePokemon;
			})
		);
...
pokemonsRouter.get('/id', async (req, res) => {
	...
	try {
		...
		const secondEvolution = evolutionChain.chain.evolves_to[0] ? evolutionChain.chain.evolves_to[0].species.name : null;

		let thirdEvolution;
		if (secondEvolution) {
			thirdEvolution =
				evolutionChain.chain.evolves_to[0].evolves_to[0] && secondEvolution
		...
		const damageRelations = await Promise.all(
			pokemon.types.map(async (item): Promise<Object> => {
